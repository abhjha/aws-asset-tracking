...
export class MapContainer extends React.Component {
	...
	constructor(props) {
        ...
        this.state = {
            ...
            //[Factory Name, Lat, Long, Status, OEE, Weekly Total, Weekly Order, Daily Total, Daily Order]
            markers : {
               "FACT01":["Factory One","37.338208","-121.886329","critical","93.23",500, 363, 60,28], 
    ...
    static getDerivedStateFromProps(props){
        let allFactoryData=props.factoriesMapData;
        let factoryData={};
        for (let key in allFactoryData) {
            factoryData[key]=[allFactoryData[key][0],allFactoryData[key][13],allFactoryData[key][14],allFactoryData[key][2],allFactoryData[key][1],allFactoryData[key][17],allFactoryData[key][18],allFactoryData[key][19],allFactoryData[key][20]]
        }
        return {
           markers:factoryData
        }
    ...
    onMapLoaded(mapObject){
        ...
        for(var i in markers){
            var icon = {
                //url: "/images/markers/normal.png", // url
    ...
    mouseEvents(currentMarker, currMarkerCount, name, maps, markers, map){
        ...
        maps.event.addListener(currentMarker, 'click', (function(currentMarker, currMarkerCount, maps) {
            //that.props.triggerNavigation('FACT01');
        })(currentMarker, currMarkerCount, maps));
    }
